...
# -------------------------------
# 5. Tokenize datasets
#    We combine prompt + label into a single 'text' sequence
#    and let DataCollatorForLanguageModeling create labels.
# -------------------------------
def tokenize_function(batch: Dict[str, List[str]]) -> Dict[str, Any]:
    # Build full text: prompt + space + label
    texts = [
        p + " " + l
        for p, l in zip(batch["prompt"], batch["label"])
    ]
    ...
    return tokenizer(
        ...
        max_length=512,
        truncation=True,
        padding=False,  # dynamic padding later in the collator
    )
...
train_tokenized = train_data.map(
    tokenize_function,
    batched=True,
    remove_columns=train_data.column_names,  # removes prompt + label + original fields
)
...
#    - will create labels from input_ids internally
# -------------------------------
...
# -------------------------------
# 9. Save final model (your preferred way)
# -------------------------------
